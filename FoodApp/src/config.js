...
export const docsData = {
  info: `01. Introduction
  TreeShaking: Bundlers will remove the unnecessary code in out application Like, removing unwanted/unused methods/functions of a library we installed in app.
  ...
  In React, We need virtual dom for Reconcilliation [ The process of syncing Virtual DOM with the real DOM].
  Reconcilliation involves following steps:
  - Renders a component - Constructs virtual DOM -Checks for changes between DOM & V-DOM -If there are any changes, updates the specific changes -Rerenders the component. -Reconcilliation involves an algorithm that react uses to diff one DOM tree from another(Virtual DOM) which is also known as Diffing algorithm and it determines what needs to changed/unchanged in the UI.
  ...
  In create-react-app, values stored in .env should have the key value starting with REACTAPP<Varialble name>.
  ...
  Named Export export const App=()=<></>
  Default export const App=()=><></> export default App;
  These components/items can be imported in any component/js file as follows.
  
  Named Imports [ Components which are exported as named exports ] import {App} from ''
  Default Imports import App from ''
...
- inside the tailwind.config.js file, for content property, we need to add the files 
  ...
  - We will use BrowserRouter to implement the routing.
      ...
      Instead of components given by react-router-dom, we can also use createBrowserRouter 
      Function and RouterProvider [Context] to implement routing. 
  ...
  When we render the appRouter, element related to the root will be rendered and associated children
  elements will be rendered in the <Outlet/> component's place.
  
      Example: When appRouter in passed as prop to the RouteProvider component,
               at the root, AppLayout will be rendered as / points to AppLayout component.
  
               And first children in home page '/' is Body component.
               So this <Body/> component will be rendered in the place of <Outlet/>
               component in the AppLayout component.
